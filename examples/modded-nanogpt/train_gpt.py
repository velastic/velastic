...
state = BCPState(model=model.state_dict(), optimizer=[opt.state_dict() for opt in optimizers], framework="ddp", get_extra_fn=get_extra_state)
iter_mngr = IterManager(use_torch_compile=True)
extra_state = state.maybe_resume_from_ckpt()
step = 0

if extra_state is not None:
    train_steps = extra_state['train_steps']
    train_loader = extra_state['train_loader']
    torch.set_rng_state(extra_state['torch_rng_state'])
    torch.cuda.set_rng_state(extra_state['cuda_rng_state'])
    step = extra_state.get('current_step', 0)

for step in range(step, train_steps + 1):
    iter_mngr.iter()
